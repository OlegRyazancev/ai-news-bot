...
- После первого meaningful push stage-ветки агент создаёт Draft PR через `gh pr create` с `base=main`, `head=<текущая ветка>` и body на основе `.github/pull_request_template.md`. Для non-stage PR неприменимые stage-поля помечаются `N/A` с причиной.
...
- `/stage-close` выполняется в две фазы: после предварительных локальных/runtime/user/docs gates изменения push-ятся без финальной отметки этапа; после успешного CI обновляется окончательный статус этапа. Если это создаёт новый commit, статус считается кандидатом до повторного успешного CI нового HEAD; failed/pending/unavailable CI не закрывает этап, completion marks возвращаются в незавершённое состояние без reset/force push. Только после успешного финального CI PR переводится в Ready for review.
