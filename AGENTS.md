...
17. Статус этапа меняется на `✅ Завершено`, а текущий фокус roadmap переключается только внутри `/stage-close` либо после отдельного явного запроса пользователя «закрой этап».
