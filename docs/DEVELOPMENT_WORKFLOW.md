...
7. **Build/Test Verification** — выполняются build, lint и применимые тесты.
8. **Runtime Verification** — фактически проверяются внешние интеграции и runtime-поведение этапа.
9. **Закрытие checklist** — обязательные пункты отмечаются выполненными только по подтверждённым результатам.
10. **Синхронизация документации** — актуализируются только документы, состояние которых действительно изменилось.
11. **Проверка Git** — просматриваются `git status` и `git diff`, проверяется отсутствие секретов и посторонних изменений.
12. **Commit / Push** — выполняются агентом только после явного решения пользователя.
...
flowchart TD
    ...
    I --> J[Build / Tests / Runtime Verification]
    J --> K{"/stage-close"}
    K -->|Не готов| E
...
4. После реализации агент запускает build/lint/применимые тесты, выполняет доступную verification, актуализирует checklist и документацию при необходимости и показывает diff.
