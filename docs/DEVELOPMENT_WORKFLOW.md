...
9. **Ручная проверка пользователем** — пользователь выполняет отдельный пошаговый раздел checklist и явно сообщает результат.
10. **Закрытие checklist** — обязательные пункты отмечаются выполненными только по подтверждённым результатам.
11. **Синхронизация документации** — актуализируются только документы, состояние которых действительно изменилось.
12. **Проверка Git** — просматриваются `git status` и `git diff`, проверяется отсутствие секретов и посторонних изменений.
13. **Commit / Push** — выполняются агентом только после явного решения пользователя.
