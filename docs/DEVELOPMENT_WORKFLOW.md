...
3. Если окончательное обновление создало новый commit, завершение считается неподтверждённым до успешного обязательного CI именно для актуального финального HEAD.
4. При `pending` агент ждёт результата, не создаёт корректирующих commits только из-за ожидания, а PR остаётся Draft. При `unavailable` сообщает конкретную проблему и не объявляет этап завершённым. При `failed` исправляет причину и повторяет необходимые локальные и CI-проверки.
5. PR body актуализируется только после повторной проверки его OPEN/head/base состояния.
6. Только `success` обязательного CI актуального финального HEAD подтверждает завершение и позволяет перевести Draft PR в Ready for review.
7. Агент сообщает пользователю, что PR готов к ручному review и **Squash and merge**.
