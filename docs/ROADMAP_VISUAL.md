...
- **Текущий этап:** 2. Сбор новостей
- **Статус:** 🟡 В работе
- **Подтверждено:** Telegram polling, базовые команды и User + UserPreferences upsert проверены в runtime; повторный `/start` не создаёт дубли. Сбор новостей ещё не реализован.
- **Следующий крупный milestone:** определить доверенные AI/LLM-источники и runtime-проверить получение и нормализацию реальных статей из RSS/API.
