...
6. Только если PR для ветки действительно отсутствует во всех состояниях, создай Draft PR через `gh pr create --draft --base main --head <current-branch>`.
...
3. Если создан новый commit, считай completion status предварительным до проверки: повторно дождись успешного `gh pr checks` именно для нового HEAD.
4. Если финальный CI failed, pending или unavailable, stage не считается окончательно закрытым и PR остаётся Draft. При failed CI верни completion marks в незавершённое состояние обычным корректирующим commit без reset/force push, исправь причину и повтори обе необходимые проверки.
5. Перед изменением PR body или состояния повторно проверь, что PR единственный, `OPEN`, head равен текущей ветке, base равен `main` и он не был closed/merged.
6. Актуализируй PR body фактическими финальными результатами.
7. Только после успешного CI актуального финального HEAD переведи Draft PR в Ready for review через `gh pr ready`.
8. Сообщи пользователю URL PR и что он готов к ручному review и предпочтительному **Squash and merge**.
